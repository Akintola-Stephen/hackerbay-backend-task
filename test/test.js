...
describe("HackerBay Stateless Microservice", () => {
  // Creating a dummy login data
  const loginDetails = { username: "Stephen", password: "Akintola" };
  // Create token variable to save user token
  let token;
  // Set various variables to be used in the application
  const imageUrl =
    "https://s3.amazonaws.com/oxfamamericaunwrapped.com/wp-content/uploads/2013/07/OAU10-53_pair_of_goats_2014-updated-image-400x400.jpg";
  const invalidImageUrl =
    "https://s3.amazonaws.com/oxfamamericaunwrapped.com/wp-content/uploads/2013/07/OAU10-53_pair_of_goats";
  const jsonObject =
  ...
  describe("Mock Authentication", () => {
    it("it should not log user in if username and password do not meet requirements", (done) => {
      request
        ...
        .send({ username: "Stephen", password: "Akintola" })
        .end((err, res) => {
